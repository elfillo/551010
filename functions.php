...
//scripts
	...
	register_nav_menu('service-repair', 'Ремонт и отделка');
	register_nav_menu('service-san', 'Сантехника');
	//add thumbnails
	add_theme_support('post-thumbnails');


	include 'parts/admin/helpers.php';
	include 'parts/admin/breadcrumbs.php';
	include 'parts/admin/custom_fields.php';

	$themeDir = get_template_directory();
	$frameworkFiles = glob(get_template_directory().'/parts/admin/constructor/*.php');
	...
	function includeStaticPart($template)
	{
		return file_get_contents("template_parts/$template.php", FILE_USE_INCLUDE_PATH);
	}

	function includePart($template, $variables = [])
	{
		ob_start();
		set_query_var( 'data', $variables );
		get_template_part("template_parts/$template");
		return ob_get_clean();
	}
